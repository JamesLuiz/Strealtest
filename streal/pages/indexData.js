...
export const StrealProvider = ({ children }) => {
  ...
  const getTokenType = async (userAddress, tokenAddress) => {
    try {
      ...
      if (window.ethereum.isConnected()) {
        ...
        // fetch the contract using signer
        const contract = fetchContract(signer);
        const token = await contract._collateralDeposited(
          userAddress,
          tokenAddress
        );
  ...
  const returnAountToMint = async (TokenAddress, amountTomint) => {
    try {
      ...
      if (window.ethereum.isConnected()) {
        ...
        // fetch the contract using signer
        const contract = fetchContract(signer);
        const data = await contract.calculateEquivalentAmountToMint(
          TokenAddress,
          amountTomint
        );
  ...
  const burnForCommerce = async (amount) => {
    try {
      ...
      console.error(
        "could not burn, check parameters and make sure your're the owner of the contract"
      );
    }
  };
  ...
  const increaseAllowance = async (addressUser, amount) => {
    try {
      ...
      if (window.ethereum.isConnected()) {
        ...
        // call the approve function
        const data = await contract.increaseAllowance(
          addressUser,
          amount * 1e18
        );
        const receipt = data.wait();
  ...
  const redeemCollateralForStreal = async (tokenAddress, amount) => {
    try {
      ...
      if (window.ethereum.isConnected()) {
        ...
        // call the approve function
        const data = await contract.redeemCollateralForStreal(
          tokenAddress,
          amount
        );
        const receipt = data.wait();
  ...
  const decreaseAllowance = async (spenderAddress, amount) => {
    try {
      ...
      if (window.ethereum.isConnected()) {
        ...
        // call the approve function
        const data = await contract.decreaseAllowance(
          spenderAddress,
          amount * 1e18
        );
        const receipt = data.wait();
  ...
  const depositStrealAndGetToken = async (
    ...
    try {
      ...
      if (window.ethereum.isConnected()) {
        ...
        // call the approve function
        const data = await contract.depositStrealAndGetToken(
          amountStreal,
          TokenCollateralAddress
        );
        const receipt = data.wait();
